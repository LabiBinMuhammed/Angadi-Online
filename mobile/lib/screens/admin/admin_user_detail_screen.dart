...
/// Admin User Detail – matches Next.js /admin/users/[userId] design
...
class _AdminUserDetailScreenState extends State<AdminUserDetailScreen> {
  ...
  Future<void> _load() async {
    final results = await Future.wait([
      supabase.from('users').select('*, user_profiles(email, profile_image_url, preferred_language, gender)').eq('id', widget.userId).single(),
      ...
      supabase.from('shop_owners').select('shops(id, name)').eq('user_id', widget.userId),
    ]);
    if (mounted) {
      setState(() {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(phone, style: const TextStyle(color: kNeutral500, fontSize: 14)),
                        ...
                        // Info rows
                        if (email != null) ...[
                          _InfoRow(icon: HugeIcons.strokeRoundedMail01, label: 'Email', value: email),
                          const SizedBox(height: 10),
                        ],
                        if (lang != null) ...[
                          _InfoRow(icon: HugeIcons.strokeRoundedGlobe, label: 'Language', value: lang),
                          const SizedBox(height: 10),
                        ],
                        if (gender != null) ...[
                          _InfoRow(icon: HugeIcons.strokeRoundedUser, label: 'Gender', value: gender),
                          const SizedBox(height: 10),
                        ],
                        _InfoRow(icon: HugeIcons.strokeRoundedCalendar01, label: 'Joined', value: _formatDate(_user?['created_at'])),
                        const SizedBox(height: 10),
                        _InfoRow(
                  ...
                  if (_shops.isNotEmpty) ...[
                    _SectionHeader(HugeIcons.strokeRoundedStore01, 'Shops Owned (${_shops.length})'),
                    const SizedBox(height: 10),
                    Container(
                      decoration: BoxDecoration(
                        color: Colors.white,
                          ...
                          return Column(
                            children: [
                              ListTile(
                                leading: Container(
                                  width: 36, height: 36,
                                  decoration: BoxDecoration(color: kBrand100, borderRadius: BorderRadius.circular(8)),
                                ...
                                trailing: GestureDetector(
                                  ...
                                  child: Container(
                                    ...
                                    child: const Text('View →', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: kBrand500)),
                                  ),
                                ),
                              ),
                              if (i < _shops.length - 1) const Divider(height: 1, indent: 16),
                            ],
                          );
                        }).toList(),
                      ),
                  ...
                  // ─── Recent Orders ────────────────────────────────────────
                  _SectionHeader(HugeIcons.strokeRoundedShoppingBag01, 'Recent Orders'),
                  const SizedBox(height: 10),
                  if (_orders.isEmpty)
                    Container(
                      ...
                      child: const Center(
                        child: Text('No orders yet.', style: TextStyle(color: kNeutral400, fontSize: 14)),
                      ),
                    )
                  else
                    Container(
                        ...
                        children: _orders.asMap().entries.map((e) {
                          ...
                          return Column(
                            children: [
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            Text(shopName, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                                            const SizedBox(height: 2),
                                            Row(
                                              children: [
                                                Text('#$shortId', style: const TextStyle(color: kNeutral400, fontSize: 12)),
                                      ...
                                      Column(
                                        ...
                                        children: [
                                          Text(priceText, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
                                          Text(_formatDate(o['created_at']), style: const TextStyle(fontSize: 11, color: kNeutral400)),
                                        ],
                                      ),
                                    ],
                                  ),
                              ...
                              if (i < _orders.length - 1) const Divider(height: 1, indent: 16),
                            ],
                          );
                        }).toList(),
                      ),
  ...
  Widget build(BuildContext context) => Row(
    children: [
      ...
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label, style: const TextStyle(fontSize: 11, color: kNeutral400)),
...
class _RoleBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Text(role.replaceAll('_', ' '), style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: color)),
  );
}
