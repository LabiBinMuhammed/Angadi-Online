  ...
  if (orders.length === 0) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: '24px', fontWeight: 800, color: '#1a1a1a', margin: '0 0 8px' }}>No notifications</h2>
        <p style={{ fontSize: '15px', color: '#777', margin: 0, fontWeight: 500 }}>You're all caught up!</p>
      </div>
    )
  }
        ...
        return (
          <Link
            ...
            style={{ 
              ...
              border: isUnread ? '2px solid #4cd964' : '2px solid transparent',
          ...
          >
            ...
            <div style={{ flex: 1, paddingTop: '4px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#1a1a1a', margin: '0 0 4px' }}>{order.shops?.name ?? 'Shop'}</h3>
              <p style={{ fontSize: '14px', color: '#555', margin: '0 0 8px', fontWeight: 500 }}>{st.label}</p>
              <span style={{ fontSize: '12px', color: '#999', fontWeight: 600 }}>{timeAgo(order.created_at)}</span>
            </div>
          </Link>
        )
      })}
