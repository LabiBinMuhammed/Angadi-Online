  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1.5rem' }}>
        <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg, var(--wa-teal), var(--wa-green))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem', color: '#fff', fontWeight: 700 }}>
      ...
      <div className="wa-list" style={{ marginBottom: '1.5rem', maxWidth: 540 }}>
        {[
          { icon: '📧', label: 'Email',    val: u.user_profiles?.email },
          { icon: '🌐', label: 'Language', val: u.user_profiles?.preferred_language },
          { icon: '⚧️', label: 'Gender',   val: u.user_profiles?.gender },
          { icon: '📅', label: 'Joined',   val: new Date(u.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }) },
          { icon: u.is_active ? '🟢' : '🔴', label: 'Status', val: u.is_active ? 'Active' : 'Inactive' },
        ].map(row => (
        ...
        <div className="wa-list">
          {(orders as any[]).map(o => (
            <Link key={o.id} href={`/admin/orders/${o.id}`} id={`udetail-order-${o.id}`} className="wa-list-item">
              <div className="wa-avatar" style={{ background: 'var(--neutral-100)', color: 'var(--text-base)', fontSize: '1rem' }}>📦</div>
              <div className="wa-item-body">
