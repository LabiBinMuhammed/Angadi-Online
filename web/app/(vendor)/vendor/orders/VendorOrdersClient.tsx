...
const STATUS_BADGE: Record<string, string> = {
  pending: 'vp-badge-warning', packing: 'vp-badge-info',
  delivering: 'vp-badge-info', delivered: 'vp-badge-success', cancelled: 'vp-badge-danger',
}
    ...
    <div className="vp-card">
      ...
      {filtered.length === 0 ? (
        <div style={{ padding: '4rem 2rem', textAlign: 'center', color: '#64748b' }}>
          ...
          <p style={{ fontSize: '0.9rem', marginTop: '0.5rem' }}>Try adjusting your search or filter</p>
        </div>
      ) : (
            ...
            <tbody>
              {filtered.map(order => (
                <tr key={order.id} id={`vorder-${order.id}`}>
                  <td style={{ color: '#94a3b8', fontSize: '0.85rem', fontFamily: 'monospace' }}>#{order.id.slice(0, 8)}</td>
                  <td>
