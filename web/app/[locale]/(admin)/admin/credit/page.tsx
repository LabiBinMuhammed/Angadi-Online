...
import { CreditCard, CheckCircle2, AlertOctagon } from 'lucide-react'
    ...
    <>
      ...
      <div className="stat-grid" style={{ marginBottom: '1.5rem' }}>
        <div className="stat-card">
          <span className="stat-card-icon" style={{ display: 'flex', alignItems: 'center', color: 'var(--wa-green-dark)' }}><CreditCard size={28} /></span>
          <span className="stat-card-value">₹{totalUsed.toFixed(0)}</span>
          <span className="stat-card-label">Total Credit Used</span>
        </div>
        <div className="stat-card">
          <span className="stat-card-icon" style={{ display: 'flex', alignItems: 'center', color: 'var(--wa-green-dark)' }}><CheckCircle2 size={28} /></span>
          <span className="stat-card-value">{enabled}</span>
          <span className="stat-card-label">Enabled</span>
        </div>
        <div className="stat-card">
          <span className="stat-card-icon" style={{ display: 'flex', alignItems: 'center', color: 'var(--danger)' }}><AlertOctagon size={28} /></span>
          <span className="stat-card-value">{blocked}</span>
          ...
          <tbody>
            {(credits ?? []).map((c: any) => (
              <tr key={c.id} id={`admin-credit-${c.id}`}>
                <td>
                  <p className="font-medium" style={{ margin: 0 }}>{c.users?.name ?? '—'}</p>
                  <p className="text-sm text-muted" style={{ margin: 0 }}>{c.users?.phone}</p>
                </td>
